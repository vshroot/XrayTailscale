...
grep -q 'type=xhttp&path=.*&mode=stream-one' xraytailscale || fail "raw XHTTP VLESS URLs must include mode=stream-one"
! grep -q 'type=xhttp&path=.*&host=.*&mode=stream-one' xraytailscale || fail "raw XHTTP VLESS URLs must not force Host header"
...
grep -q 'xhttp_path_sync_after_multiroute_fix_2026' xraytailscale || fail "missing post-fix XHTTP path sync migration marker"
