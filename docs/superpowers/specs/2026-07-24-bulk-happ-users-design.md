...
If existing users reference stale ports and their subscription endpoint returns `410 Gone`, `repair_stale_subscription_profiles_core` rewrites stale profile routes to the current seed routes, preserves `uuid`/`sub_token`, adds the UUIDs to live inbounds, and restarts Xray once. The `bulk-repair` CLI command remains a compatibility alias for this broader repair path.
